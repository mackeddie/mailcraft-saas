...
import { InsertUser, users, campaigns, subscribers, segments, emailTemplates, emailAnalytics } from "../drizzle/schema";
...
// Analytics queries
export async function getAnalyticsByCampaignId(campaignId: number) {
  const db = await getDb();
  if (!db) return [];
  return db.select().from(emailAnalytics).where(eq(emailAnalytics.campaignId, campaignId));
}
